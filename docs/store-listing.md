...
```text
暗号化プライベート保護・コマンドパレット・アクセス分析を備えた高機能ブックマークマネージャー。外部通信ゼロで完全ローカル動作。
```
*(132文字以内)*
...
2. 🔒 特別なタグ「プライベート」と保護
・完全な隠蔽: プライベートタグが付いたブックマークは、通常の一覧・タグ・検索から完全に除外されます。
・マスターパスワード保護: Web Crypto API（PBKDF2 / SHA-256）を用いた強固なローカルハッシュで保護。
・シークレットウィンドウ自動起動: プライベートブックマークを開く際は、Chrome のシークレットモード（Incognito Window）で自動的に開きます。
...
・パレット内プライベート解除: パスワードを入力することで、パレット上でも秘密のブックマークを瞬時に検索・起動可能。
...
💡 初回利用時の推奨設定
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
プライベートタグ付きブックマークを Chrome のシークレットウィンドウで自動起動するため、以下の設定を推奨します：
...
| **4** | **プライベート保護** | プライベートタグ付きブックマークのパスワード認証モーダル、およびシークレットウィンドウで開く動作。<br>*「マスターパスワード保護とシークレットウィンドウ自動起動」* |
